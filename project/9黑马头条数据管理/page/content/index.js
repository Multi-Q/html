...
async function setArticleList() {
    try {
        ...
        <tr >
          <td>
            <img src= ${item.cover.type === 0 ? 'https://img2.baidu.com/it/u=2640406343,1419332367&amp;fm=253&amp;fmt=auto&amp;app=138&amp;f=JPEG?w=708&amp;h=500' : item.cover.images[0]}  alt="${item.title}">
          </td>
          <td>${item.title}</td>
          <td>
        ...
        document.querySelector(".art-list").innerHTML = articles;
...
document.querySelector(".pagination .next").addEventListener("click", e => {
    if (queryObj.page < Math.ceil(totalCount / queryObj.per_page)) {
        queryObj.page++;
        setArticleList();
        document.querySelector(".pagination .page-now").innerHTML = `第${queryObj.page}页`;
    }
});
...
document.querySelector(".art-list").addEventListener("click", async e => {
    if (e.target.classList.contains("edit")) {
        const articleId=e.target.parentNode.dataset.id;
        location.assign(`../publish/index.html?id=${articleId}`);
